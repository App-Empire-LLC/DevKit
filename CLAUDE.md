...
- `src/aidevkit/` — the Python package
  ...
  - `bootstrap.py` / `doctor.py` / `setup.py` — one module per subcommand
...
- `tests/` — empty; populated by #22

## Conventions

- All `git` / `gh` calls go through `aidevkit.util.run` (one subprocess seam, `shell=False`). Tests in #22 can mock at this one point.
- Exit codes are preserved from the bash-era: `0` success, `2` usage, `10` repos-missing, `11` workspace-exists, `12` dep-missing, `13` repo-not-found.
